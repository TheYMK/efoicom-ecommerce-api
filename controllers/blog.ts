...
export const create = async (req: Request, res: Response) => {
	...
	form.parse(req, (err, fields, files) => {
		...
		blog.save((err, result) => {
			if (err) {
				throw new BadRequestError('Error occured during attempt to save a new blog');
			}
	...
	Blog.find({})
		...
		.exec((err, data) => {
			...
			Blogcategory.find({}).exec((err, c) => {
				if (err) {
					throw new BadRequestError(`Failed to fetch blog categories`);
				}
...
const handleBlogCategoryQuery = async (req: Request, res: Response, blogcategory: any) => {
	try {
		if (blogcategory === 'all') {
		...
		throw new BadRequestError('Failed to fetch blogs based on categories selection');
	}
};
